...
This Home Assistant add-on provides local WebRTC, RTSP, RTMP, and HLS access for Wyze cameras. The `4.2` release line keeps the established bridge path, bundles a native `go2rtc` sidecar for Home Assistant, and documents the model-specific limits now known for newer Wyze cameras. The `4.2.1` patch line fixed the sidecar `:8555` conflict on Home Assistant hosts, `4.2.2` hardens the MQTT motion path used by Home Assistant and Scrypted workflows, `4.2.4` tightens feed-selection precedence so explicit `CAM_OPTIONS` values beat stale saved settings without spinning up the wrong bridge substream, `4.2.5` refreshes preserved native Wyze aliases with the current helper URLs at startup, `4.2.6` closes the remaining ingress UI asset gap while making native alias prep follow the bridge's live published camera catalog, and `4.2.7` keeps native-only V3 Pro SD feeds available for Home Assistant SD-only setups.
