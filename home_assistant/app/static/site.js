 ...
 */
async function update_img(oldUrl, options = {}) {
  const useImg = options.useImg || false;
  const refreshMode = options.refreshMode || "snapshot";
  let [cam, ext] = oldUrl.split("/").pop().split("?")[0].split(".");
  let newUrl = "snapshot/" + cam + "." + ext + "?" + Date.now();
  if (refreshMode === "thumb") {
    newUrl = "thumb/" + cam + "." + ext + "?" + Date.now();
  } else if (refreshMode === "img" || useImg || ext == "svg") {
    newUrl = "img/" + cam + "." + ext;
  }
...
function refresh_imgs() {
  document.querySelectorAll(".refresh_img").forEach(async function (image) {
    ...
    await update_img(url, {
      refreshMode: image.dataset.refreshMode,
      useImg: !(CameraEnabled && (!CameraBattery || CameraConnected)),
    });
  });
}
