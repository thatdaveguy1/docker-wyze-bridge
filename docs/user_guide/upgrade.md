...
This guide covers upgrading to **Docker Wyze Bridge V4.0.1** from earlier versions or from other forks.

## Upgrading from Upstream (idisposable/docker-wyze-bridge)

The V4.0.1 release keeps the V4 platform work intact while tightening the Home Assistant login defaults and support wording.
...
2.  **Stop the old add-on:** If you were using the upstream version, stop it before installing the V4.0.1 release to avoid port conflicts.
...
4.  **Install V4.0.1:** Install the new **Docker Wyze Bridge (V4.0.1)** add-on and paste your saved configuration into the configuration tab.
...
### "Port already in use"
- **Problem:** The V4.0.1 add-on fails to start with a port conflict error.
- **Solution:** 
